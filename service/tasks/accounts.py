...
def remove_empty_networks_for(provider_id):
    ...
    known_project_names = Credential.objects.filter(key='ex_project_name'
                                                   ).values_list(
                                                       'value', flat=True
                                                   )
    ...
    for project in projects_with_networks:
        ...
        for network in networks:
            ...
            celery_logger.debug(
                "Checking if network %s is in use" % network_name
            )
            ...
            identity = Identity.objects.filter(
                ...
                credential__value=project
            ).filter(
                credential__key='key', credential__value=user
            ).first()
            if not identity:
                celery_logger.warn(
            ...
            try:
                ...
                celery_logger.exception(
                    "Neutron unable to remove project"
...
def remove_empty_networks():
    celery_logger.debug(
        "remove_empty_networks task started at %s." % datetime.now()
    )
...
def running_instances(network_name, all_instances):
    for instance in all_instances:
        if network_name in instance.extra['addresses'].keys():
            ...
            celery_logger.debug(
                "Network %s is in use, Active Instance:%s" %
