...
def _update_volume_metadata(esh_driver, esh_volume, metadata={}):
    """
...
def restrict_size_by_image(size, image):
    ...
    if size > image_size + 4:
        raise exceptions.VolumeError(
            ...
            "more than 4GB greater than the size of the image:(%s GB)" % size
        )
...
def create_bootable_volume(
    ...
    **kwargs passed as data to boot_volume_instance
    ...
    core_instance = boot_volume_instance(
        driver, user, identity, source, size, name, **kwargs
    )
...
def attach_volume(driver, instance_id, volume_id, device_choice=None):
    ...
    return driver.attach_volume(instance, volume, device_choice)
