...
def add_user_urls(request, provider_uuid, identity_uuid):
    data = {
        'group-list':
            reverse('api:v1:group-list', request=request),
        'tag-list':
            reverse('api:v1:tag-list', request=request),
        'provider-list':
            reverse('api:v1:provider-list', request=request),
        'occupancy':
            reverse(
                'api:v1:occupancy', args=(provider_uuid, ), request=request
            ),
        'hypervisor':
            reverse(
                'api:v1:hypervisor', args=(provider_uuid, ), request=request
            ),
        'identity-list':
            reverse(
                'api:v1:identity-list', args=(provider_uuid, ), request=request
            ),
        'volume-list':
            reverse(
                'api:v1:volume-list',
        ...
        'machine-history-list':
            reverse(
                'api:v1:machine-history',
                args=(provider_uuid, identity_uuid),
                request=request
            ),
        'instance-history-list':
            reverse(
                'api:v1:instance-history',
                args=(provider_uuid, identity_uuid),
                request=request
            ),
        'instance-list':
        ...
        'profile':
            reverse('api:v1:profile', request=request)
    }
    return data
    ...
    def get(self, request, provider_uuid, identity_uuid, action=None):
        ...
        if not action:
            return failure_response(
                status.HTTP_400_BAD_REQUEST, 'Action is not supported.'
            )
        try:
            esh_driver = prepare_driver(request, provider_uuid, identity_uuid)
        except ProviderNotActive as pna:
