...
from api.exceptions import (
    invalid_creds, malformed_response, connection_failure, failure_response,
    inactive_provider, invalid_provider_identity
)
...
def provider_filtered_machines(
    request, provider_uuid, identity_uuid, request_user=None
):
    """
...
def list_filtered_machines(esh_driver, provider_uuid, request_user=None):
    ...
    filtered_machine_list = [
        core_mach
        for core_mach in core_machine_list if filter_core_machine(core_mach)
    ]
    sorted_machine_list = sorted(
        filtered_machine_list, cmp=compare_core_machines
    )
...
class Machine(AuthAPIView):
    ...
    def patch(self, request, provider_uuid, identity_uuid, machine_id):
        ...
        (Lookup using the given provider/identity)
        """
        return self._update_machine(
            request, provider_uuid, identity_uuid, machine_id
        )
