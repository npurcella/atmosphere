...
from core.models import (
    Group, MaintenanceRecord, AtmosphereUser, ExternalLink, Volume, Instance,
    Project, Identity
)
...
class ProjectMemberRequired(permissions.BasePermission):
    ...
    def v1_membership_test(self, request, view):
        ...
        if instance_id:
            ...
            logger.debug(
                "Cannot test membership without a kwarg:'instance_id', 'identity_id', or 'volume_id'"
            )
            return True
    ...
    def test_link_permissions(self, user, link_id, is_leader=False):
        ...
        return ExternalLink.shared_with_user(
            user, is_leader=is_leader
        ).filter(**link_kwargs).exists()
    ...
    def test_volume_permissions(self, user, volume_id, is_leader=False):
        ...
        return Volume.shared_with_user(
            user, is_leader=is_leader
        ).filter(**volume_kwargs).exists()
    ...
    def test_instance_permissions(self, user, instance_id, is_leader=False):
        ...
        return Instance.shared_with_user(
            user, is_leader=is_leader
        ).filter(**instance_kwargs).exists()

    def test_project_permissions(
        self, user, request_method, project_id, is_leader=False
    ):
        project_kwargs = {}
        ...
        return Project.shared_with_user(
            user, is_leader=is_leader
        ).filter(**project_kwargs).exists()
...
class ProjectLeaderRequired(ProjectMemberRequired):
    ...
    def test_project_permissions(self, user, request_method, project_id):
        return super(ProjectLeaderRequired, self).test_project_permissions(
            user, request_method, project_id, is_leader=True
        )

    def test_link_permissions(self, user, link_id):
        return super(ProjectLeaderRequired, self).test_link_permissions(
            user, link_id, is_leader=True
        )

    def test_volume_permissions(self, user, volume_id):
        return super(ProjectLeaderRequired, self).test_volume_permissions(
            user, volume_id, is_leader=True
        )

    def test_identity_permissions(self, user, identity_id):
        return super(ProjectLeaderRequired, self).test_identity_permissions(
            user, identity_id, is_leader=True
        )

    def test_instance_permissions(self, user, instance_id):
        return super(ProjectLeaderRequired, self).test_instance_permissions(
            user, instance_id, is_leader=True
        )
...
class UserListAdminQueryable(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (method_is_safe and no_bad_params)
