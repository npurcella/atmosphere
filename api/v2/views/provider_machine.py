...
from core.query import (
    only_current_source, only_public_providers, user_provider_machine_set,
    in_provider_list
)
...
def get_admin_machines(user):
    ...
    for user in admin_users:
        machine_ids.extend(
            user.source_set.filter(providermachine__isnull=False
                                  ).values_list('providermachine', flat=True)
        )
    admin_list = ProviderMachine.objects.filter(
        only_current_source(), id__in=machine_ids
    )
    return admin_list
...
class ProviderMachineViewSet(MultipleFieldLookup, OwnerUpdateViewSet):
    ...
    search_fields = (
        'application_version__id', 'application_version__application__id',
        'instance_source__created_by__username'
    )
    ...
    def update_metadata(self, request, pk=None):
        ...
        try:
            ...
            logger.exception(
                "Error occurred updating v2 provider_machine metadata"
            )
    ...
    def get_queryset(self):
        ...
        queryset = (public_set | shared_set | my_set | admin_set
                   ).distinct().order_by('-instance_source__start_date')

        if not isinstance(request_user, AnonymousUser):
            launchable_providers = request_user.current_providers.filter(
                active=True
            )
