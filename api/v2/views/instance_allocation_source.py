...
from core.models import (
    InstanceAllocationSourceSnapshot, EventTable, Instance, AllocationSource,
...
class InstanceAllocationSourceViewSet(AuthModelViewSet):
    ...
    def get_queryset(self):
        ...
        return InstanceAllocationSourceSnapshot.objects.all(
        )    # filter(user__uuid=user)
    ...
    def _validate_data(self, request_user, request_data):
        ...
        if 'instance_id' not in request_data:
            raise Exception(
                'Missing Instance Provider Alias from request data '
            )

        if 'allocation_source_name' not in request_data:
            raise Exception(
                'Missing Allocation Source Source ID from request data '
            )
    ...
    def _for_validate_userallocationsource(self, request_user, request_data):
        instance = Instance.objects.filter(
            provider_alias=request_data['instance_id']
        ).last()
        ...
        if instance.created_by != request_user:
            raise Exception(
                ...
                (request_data['instance_id'], request_user.username)
            )

        if not UserAllocationSource.objects.filter(
            user=request_user, allocation_source=allocation_source
        ):
            raise Exception(
        ...
        if InstanceAllocationSourceSnapshot.objects.filter(
            instance=instance, allocation_source=allocation_source
        ):
            raise Exception(
                'Instance %s is already assigned to Allocation Source %s' % (
                    request_data['instance_id'],
                    request_data['allocation_source_name']
                )
            )
