...
def bad_request(errors, prefix="", status_code=None):
    ...
    error_str = ''.join(
        ["%s%s:%s" % (prefix, key, val[0]) for (key, val) in errors.items()]
    )
    error_map = {
        "errors": [{
            "code": status_code,
    ...
    }    # This is an expected format by atmo-airport.
    return Response(error_map, status=status_code)
...
def malformed_response(provider_id, identity_id):
    logger.warn(
        'Server provided bad response. Provider-id:%s Identity-id:%s' %
        (provider_id, identity_id)
    )
    return failure_response(
        status.HTTP_500_INTERNAL_SERVER_ERROR, "Cloud Communications Error --"
...
def invalid_provider(provider_id):
    ...
    return failure_response(status.HTTP_401_UNAUTHORIZED, log_message)
...
def invalid_creds(provider_id, identity_id):
    ...
    return failure_response(
        status.HTTP_401_UNAUTHORIZED, 'Identity/Provider Authentication Failed'
    )
...
def connection_failure(provider_id, identity_id=None):
    logger.warn(
        ...
        'Provider-id:%s Identity-id:%s' % (provider_id, identity_id)
    )
    return failure_response(
...
def instance_not_found(instance_id):
    return failure_response(
        status.HTTP_404_NOT_FOUND, 'Instance %s does not exist' % instance_id
    )


def size_not_available(sna_exception):
    return failure_response(
        status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, sna_exception.message
    )


def inactive_provider(provider_exception):
    return failure_response(
        status.HTTP_409_CONFLICT, provider_exception.message
    )


def over_capacity(capacity_exception):
    return failure_response(
        status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, capacity_exception.message
    )


def under_threshold(threshold_exception):
    return failure_response(
        status.HTTP_400_BAD_REQUEST, threshold_exception.message
    )
...
def mount_failed(exception):
    return failure_response(status.HTTP_409_CONFLICT, exception.message)


def over_allocation(allocation_exception):
    return failure_response(
        status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, allocation_exception.message
    )
