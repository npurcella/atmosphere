...
class AccountCreationPlugin(object):
    ...
    def find_accounts(
        self, provider, account_user, group_name, username, project_name,
        **kwargs
    ):
        ...
        member = GroupMembership.objects.filter(
            user__username=account_user, group__name=group_name
        )
        if not member:
        ...
        return group.identities.filter(
            ...
            provider=provider
        ).filter(contains_credential('key', username))
...
class DirectOpenstackAccount(AccountCreationPlugin):
    ...
    def create_accounts(self, provider, username, force=False):
        ...
        identities = Identity.objects.filter(
            provider=provider, created_by__username=username
        )
        ...
        for identity in identities:
            ...
            try:
                has_allocations = AllocationSourcePluginManager.ensure_user_allocation_sources(
                    user
                )
            ...
            if settings.AUTO_CREATE_NEW_PROJECTS:
                ...
                projects = Project.objects.filter(
                    created_by=user, name=project_name
                )
                ...
                if not has_projects and membership:
                    ...
                    logger.info(
                        'Creating new project for %s: "%s"', user, project_name
                    )
