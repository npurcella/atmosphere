...
class JetstreamSpecialAllocationQuota(
    atmosphere.plugins.quota.default_quota.DefaultQuotaPlugin
):
    def get_default_quota(self, user, provider):
        ...
        """
        driver = TASAPIDriver()
        special_allocation_sources = getattr(
            django.conf.settings, 'SPECIAL_ALLOCATION_SOURCES', {}
        )
        default_quota = None
        try:
            ...
            if project_allocations and len(
                project_allocations
            ) == 1 and project_allocations[0].name in special_allocation_sources:
                import core.models
