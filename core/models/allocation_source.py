...
class AllocationSource(models.Model):
    ...
    def for_user(cls, user):
        source_ids = UserAllocationSource.objects.filter(user=user).values_list(
            'allocation_source', flat=True
        )
    ...
    def time_remaining(self, user=None):
        ...
        time_shared_allocations = getattr(
            settings, 'SPECIAL_ALLOCATION_SOURCES', {}
        )
        if user and self.name in time_shared_allocations.keys():
            try:
                compute_allowed = time_shared_allocations[self.name
                                                         ]['compute_allowed']
            except:
                raise Exception(
            ...
            try:
                ...
                logger.exception(
                    'User allocation snapshot does not exist anymore (or yet), so returning -1'
                )
                return -1
        else:
...
class UserAllocationSource(models.Model):
    ...
    user = models.ForeignKey(
        "AtmosphereUser", related_name="user_allocation_sources"
    )
    ...
    allocation_source = models.ForeignKey(
        AllocationSource, related_name="users"
    )
...
class UserAllocationSnapshot(models.Model):
    ...
    """
    user = models.ForeignKey(
        "AtmosphereUser", related_name="user_allocation_snapshots"
    )
    allocation_source = models.ForeignKey(
        AllocationSource, related_name="user_allocation_snapshots"
    )
...
class AllocationSourceSnapshot(models.Model):
    allocation_source = models.OneToOneField(
        AllocationSource, related_name="snapshot"
    )
    updated = models.DateTimeField(auto_now=True)
    ...
    compute_used = models.DecimalField(max_digits=19, decimal_places=3)
    compute_allowed = models.DecimalField(
...
def total_usage(
    ...
    if compute_used_total > 0:
        logger.info(
            "Total usage for User %s with AllocationSource %s from %s-%s = %s" %
            (
                username, allocation_source_name, start_date, end_date,
                compute_used_total
            )
        )
    if burn_rate:
        burn_rate_total = 0 if len(user_allocation
                                  ) < 1 else user_allocation[-1]['burn_rate']
        if burn_rate_total != 0:
            logger.info(
                "User %s with AllocationSource %s Burn Rate: %s" %
...
def get_allocation_source_object(source_id):
    if not source_id:
        raise Exception(
            'No source_id provided in _get_allocation_source_object method'
        )
